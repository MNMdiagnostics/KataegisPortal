katPoint <- function(data, sample = "sample", min.mut = 6, max.dis = 1000,
  ...
  if (build == "hg19") {
    chr.arm <- c(
      1.25e+08, 93300000, 9.1e+07, 50400000, 48400000,
    ...
    chr.arm <- c(
      124300000, 93300000, 91700000, 50700000, 47700000,
    ...
    chr.arm <- c(
      123400000, 93900000, 90900000, 5e+07, 48800000,
  ...
  for (i in 1:num) {
    ...
    if (data$dis[i + 1] <= max.dis) {
      ...
      if (mutnum >= min.mut) {
        ...
        chr.n <- gsub(pattern = "chr", replacement = "", x = data$chr[i], fixed = TRUE)
        chr.n <- gsub(pattern = "X", replacement = "23", x = chr.n, fixed = TRUE)
        chr.n <- gsub(pattern = "Y", replacement = "24", x = chr.n, fixed = TRUE)
        ...
        katPoint[i, 1:8] <- c(
          sample, data$chr[i], data$pos[i - mutnum + 1], data$pos[i], arm, len,
          mutnum, round(Cmutnum / mutnum, 3)
        )
      }
      mutnum <- 1
      Cmutnum <- 0
  ...
  for (i in 1:dim(katPoint.out)[1]) {
    if (as.numeric(as.character(katPoint.out$"weight.C>X"[i])) < 0.8) {
      ...
      katPoint.out$confidence[i] <- length(which(subset(
        katPoint.out,
        as.numeric(as.character(katPoint.out$"weight.C>X")) >= 0.8
      )$chrom == katPoint.out$chrom[i]))
  ...
  if (!is.null(txdb)) {
    gr <- GRanges(
      seqnames = Rle(katPoint.out$chrom),
      ranges = IRanges(
        start = as.numeric(as.character(katPoint.out$start)),
        end = as.numeric(as.character(katPoint.out$end))
      )
    )
  ...
  message(paste(dim(katPoint.out)[1], "potential kataegis events identified", sep = " "))
